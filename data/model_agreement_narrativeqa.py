#!/usr/bin/env python3
"""Run 3-model soft-label agreement over NarrativeQA samples."""
...
def normalize_text(text: str) -> str:
    ...
    # Keep letters/digits/CJK/space only for stable exact-ish matching.
...
def bm25_top_k(chunks: List[str], query: str, top_k: int = 12) -> List[int]:
    if not chunks:
        return []
    if top_k <= 0:
        return []
...
def dense_top_k(
    ...
    """Retrieve top-k chunks by dense embedding cosine similarity."""
...
def retrieve_candidate_chunk_ids(
    ...
    """Fuse BM25 and Dense retrieval results by union."""
...
def load_samples(max_n: int, split: str = "validation") -> List[Dict[str, Any]]:
    out = load_narrativeqa_subset(train_size=max_n, validation_size=max_n)
    if split not in out:
        raise ValueError(f"split {split} not found in loaded data")
    samples = out[split][:max_n]
    return samples


def get_answer(client: OpenAICompatibleClient, numbered_chunks: str, question: str) -> str:
    messages = [
        {
            "role": "system",
            "content": "你是一个严格 JSON 输出助手。",
        },
        {"role": "user", "content": prompt_for_qa(numbered_chunks, question)},
    ]
    return client.generate(messages=messages, temperature=0.0, max_tokens=1024)
...
def validate_quoted_evidence(
    ...
    """Verify each quoted evidence sentence exists in its corresponding chunk."""
    ...
    all_verified = all(verified_per_quote)
    return all_verified, verified_per_quote
...
def parse_structured_response(
    ...
    """Parse LLM JSON response with improved robustness.

    Fixes:
    - Strips markdown code blocks (```json ... ```)
    - Allows selected_chunk_ids length 0-2 (was 1-2)
    - Tries to extract JSON from text if direct parse fails
    """
    raw = raw.strip()
    if not raw:
    ...
    try:
        ...
        if not (0 <= len(selected_chunk_ids_parsed) <= 2):
            raise ValueError(f"selected_chunk_ids length must be 0-2, got {len(selected_chunk_ids_parsed)}")
        ...
        all_verified = False
        verified_per_quote = []
        if selected_chunk_ids is not None and chunks is not None and quoted_evidence:
...
def compute_agreement(answer_texts: List[str]) -> Dict[str, Any]:
    ...
    pairwise_agree = sum(1 for k, v in c.items() if v > 1)  # 1 means at least one pair
    return {
        "all_same": all_same,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NarrativeQA 2-model label agreement checker")
    parser.add_argument("--split", choices=["train", "validation"], default="validation")
    parser.add_argument("--max_n", type=int, default=100)
    parser.add_argument("--output", type=str, default="outputs/model_agreement_100.jsonl")
    parser.add_argument("--model1", type=str, default=os.getenv("MODEL_1", "gpt-5.4"))
    parser.add_argument("--model2", type=str, default=os.getenv("MODEL_2", "qwen3.5-plus-2026-02-15"))
    ...
    print(f"加载 NarrativeQA {args.split} 前 {args.max_n} 条数据...")
    samples = load_samples(max_n=args.max_n, split=args.split)
    ...
    for i, sample in enumerate(samples, start=1):
        ...
        for name, client in clients.items():
            try:
                raw = get_answer(client, numbered_chunks, q)
                parsed = parse_structured_response(raw, selected_chunk_ids=None, chunks=chunks)
        ...
        parse_ok = i - parse_errors
        same_chunk = summary.get("evidence_all_same", 0)

        if i % 10 == 0 or i == total:
            pct = i / total * 100
            print(
                f"[{i}/{total}] ({pct:.1f}%) "
                f"parse_ok={parse_ok}({parse_ok/i*100:.0f}%) "
                f"same_chunk={same_chunk}({same_chunk/i*100:.0f}%) "
                f"same_answer={summary['answer_all_same']}({summary['answer_all_same']/i*100:.0f}%)"
            )
    ...
    print("\n===== 统计结果 =====")
    print(f"总样本数: {total}")
    print(f"answer_all_same: {summary['answer_all_same']} ({summary['answer_all_same']/total*100:.1f}%)")
    print(f"evidence_all_same: {summary['evidence_all_same']} ({summary['evidence_all_same']/total*100:.1f}%)")
    print(f"parse_error_count: {summary.get('parse_error_count', 0)} ({summary.get('parse_error_count', 0)/total*100:.1f}%)")
